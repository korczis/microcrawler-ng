...
export default class App {
  static listModules(modules) {
    const table = new Table({
      head: ['Name', 'URL']
    });

    Object.keys(modules).forEach((moduleName) => {
      const info = modules[moduleName].info;
      table.push([
        info.name,
        info.url
      ]);
  ...
  run() {
    ...
    this.loadModules().then((modules) => {
      ...
      if (!commander.type && !commander.args.length) {
      ...
      const args = commander.args.length ? commander.args : [module.info.url];
      if (!args || args == []) {
